#include <stdio.h>
 ...
 void usar_banio(int id, int sexo) {
    ...
    sleep(rand() % 2 + 1); // Simula el uso del baño
 ...
 void* persona(void* arg) {
    ...
    // Esperar mientras:
    // 1. El baño esta lleno O
    // 2. El baño es del otro género y no está vacío
    while(((hombres_dentro || mujeres_dentro) && banio_sexo != sexo) || (banio_sexo == sexo && ((sexo == HOMBRE && hombres_dentro >= CAPACIDAD) || (sexo == MUJER && mujeres_dentro >= CAPACIDAD)))) {
...
return NULL;
 ...
 int main(){
    ...
    bool generos[15] = {1, 0, 0, 1, 0, 1, 0, 1, 1, 0, 1, 0, 0, 1, 0};
